...
def _load_map() -> Dict[str, Dict[str, Any]]:
    try:
        with _map_lock(_MAP_PATH, exclusive=False):
            if _MAP_PATH.exists():
...
def _save_map(data: Dict[str, Dict[str, Any]]) -> None:
    ...
    try:
        ...
        with _map_lock(_MAP_PATH, exclusive=True):
            with tempfile.NamedTemporaryFile(
                mode="w",
                encoding="utf-8",
                delete=False,
                dir=str(_MAP_PATH.parent),
                prefix=f".{_MAP_PATH.name}.",
                suffix=".tmp",
            ) as temp_file:
            ...
            os.replace(str(temp_path), str(_MAP_PATH))
    except Exception as exc:
...
@contextmanager
def _map_lock(path: Path, exclusive: bool) -> Any:
    lock_file = None
    try:
        lock_path = path.with_name(path.name + _MAP_LOCK_SUFFIX)
        ...
        try:
            ...
            _log_error(f"plate_registry lock failed: {exc}")
        yield
    finally:
...
def _prompt_vehicle_key(plate: str) -> Optional[str]:
    ...
    try:
        ...
        vehicle_key = simpledialog.askstring(
            "Введите vehicle_key",
            prompt,
            parent=root,
        )
